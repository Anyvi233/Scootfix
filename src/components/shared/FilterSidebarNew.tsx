...
// ── Inner component ─────────────────────────────────────────────────────────
// Uses useSearchParams(), so it MUST be a child of <Suspense>.
      ...
      <FilterSection title="Part Origin" defaultOpen={true}>
        <label className="flex items-center space-x-3 cursor-pointer">
          <input
            type="checkbox"
            checked={isOem}
            onChange={() => updateParam("oem", isOem ? null : "true")}
        ...
        <label className="flex items-center space-x-3 cursor-pointer">
          <input
            type="checkbox"
            checked={isAftermarket}
            onChange={() => updateParam("aftermarket", isAftermarket ? null : "true")}
            className="w-4 h-4 rounded border-border text-primary focus:ring-primary"
          />
        ...
        <form onSubmit={handlePriceSubmit} className="flex items-center gap-2 mt-2">
          <div className="flex-1">
            <input
              ...
              placeholder="Min ₹"
              className="w-full px-3 py-2 bg-surface text-sm border border-border rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
            />
          </div>
          ...
          <button
            type="submit"
            className="p-2 bg-primary/10 text-primary rounded-md hover:bg-primary/20"
          >
            ...
            <div className="flex items-center">
              <div className="flex items-center text-warning mr-2">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`w-3.5 h-3.5 ${i < rating ? "fill-current" : "text-border fill-border"}`}
                    viewBox="0 0 24 24"
                  >
      ...
      <FilterSection title="Availability & Offers" defaultOpen={true}>
        ...
        <label className="flex items-center space-x-3 cursor-pointer">
          <input
            type="checkbox"
            checked={hasDiscount}
            onChange={() => updateParam("discount", hasDiscount ? null : "true")}
            className="w-4 h-4 rounded border-border text-primary focus:ring-primary"
          />
          <span className="text-sm text-text-secondary flex items-center gap-1">
            On Sale{" "}
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-danger/10 text-danger font-bold ml-1">
              HOT
            </span>
          </span>
        </label>
...
// ── Public export ────────────────────────────────────────────────────────────
// Wraps FilterSidebarInner in <Suspense> so useSearchParams() works correctly.
    ...
    <Suspense
      fallback={
        <div className="w-full space-y-4 animate-pulse">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-10 bg-border/40 rounded-lg" />
          ))}
        </div>
      }
    >
      <FilterSidebarInner className={className} onClose={onClose} />
    </Suspense>
  );
}
